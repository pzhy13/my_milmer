...
class NervFormerEEGModel(nn.Module):
    def __init__(self, num_channels=32, time_length=250, output_dim=768, dropout_rate=0.3): # 参数化
        ...
        # --- 修改: 确保卷积核尺寸不超过输入尺寸 ---
        time_kernel = (1, 25) if time_length >= 25 else (1, 3) 
        ...
        self.tsconv = nn.Sequential(
            ...
            nn.BatchNorm2d(40),
            nn.ELU(),
            nn.Dropout(dropout_rate), # 使用传入的 dropout
        )
        ...
        self.self_attn_ts = nn.MultiheadAttention(embed_dim=40, num_heads=5, dropout=dropout_rate)
        self.self_attn_st = nn.MultiheadAttention(embed_dim=40, num_heads=5, dropout=dropout_rate)
        self.cross_attn = nn.MultiheadAttention(embed_dim=40, num_heads=8, dropout=dropout_rate)
        ...
        self.feed_forward = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout_rate), # 增加 Dropout
...
class MultiModalClassifier(nn.Module):
    def __init__(self, 
        ...
        if self.use_nerv_eeg:
            # --- 集成 NervFormer ---
            self.eeg_encoder = NervFormerEEGModel(
                num_channels=eeg_channels, 
                time_length=eeg_time_len, 
                output_dim=input_size,
                dropout_rate=cls_dropout_rate # 传递 dropout
            )
    ...
    def forward(self, eeg_data=None, images_data=None):
        ...
        if images_data is not None:
            ...
            for i in range(self.num_instances):
                ...
                # Swin 已被冻结，确保这里不计算梯度（尽管 requires_grad=False 已处理）
                # 但为了保险起见和显存优化，保留 torch.no_grad()
                with torch.no_grad(): 
